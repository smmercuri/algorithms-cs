...
The aim here is to consider two things -- shortest ancestral paths and outcase detection. Given any two nouns, nounA and nounB, we would like to find the shortest ancestral path (SAP) between the synsets containing these nouns. The SAP is the shortest path from nounA to nounB that passes through a common ancestor (this will be a path, but not a directed path). We generalise this notion to find the SAP between two sets of nouns. This is done in *SAP*.
