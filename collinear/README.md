...
To do this efficiently, we do the following in *FastCollinearPoints*. For each point *p*, to check if it participates in a line segment of 4 or more points, we sort all other points by the slope they make with *p*. Then we search for 3 consecutive points whose slope with *p* are all equal, then *p* and these three points lie on a line.
